...
describe('Factory Call Count Optimization', () => {
  ...
  test('factory_call_only_once_per_token', () => {
    ...
    const tokenId = BigInt.fromI32(7001)
    const liquidity1 = BigInt.fromI32(1000)
    const liquidity2 = BigInt.fromI32(500)
    const liquidity3 = BigInt.fromI32(200)
    ...
    const expectedLiquidity = liquidity1.plus(liquidity2)
    assert.fieldEquals(
      'Position',
      tokenId.toString(),
      'liquidity',
    ...
    const finalLiquidity = expectedLiquidity.minus(liquidity3)
    assert.fieldEquals(
      'Position',
      tokenId.toString(),
      'liquidity',
  ...
  test('different_tokens_each_get_one_factory_call', () => {
    ...
    const tokenId1 = BigInt.fromI32(7002)
    const tokenId2 = BigInt.fromI32(7003)
    const tokenId3 = BigInt.fromI32(7004)
  ...
  test('cached_pool_lookup_after_initial_call', () => {
    ...
    const tokenId = BigInt.fromI32(7005)
    ...
    const expectedLiquidity = initialLiquidity
      .plus(additionalLiquidity)
      .minus(BigInt.fromI32(500))
    assert.fieldEquals(
      'Position',
      tokenId.toString(),
      'liquidity',
  ...
  test('factory_call_performance_guard', () => {
    ...
    for (let tokenIndex = 0; tokenIndex < numTokens; tokenIndex++) {
      const tokenId = BigInt.fromI32(baseTokenId + tokenIndex)
    ...
    for (let opIndex = 0; opIndex < numOperations; opIndex++) {
      const tokenIndex = opIndex % numTokens // Cycle through tokens
      const tokenId = BigInt.fromI32(baseTokenId + tokenIndex)
      ...
      if (opIndex % 2 == 0) {
        ...
        if (opIndex > tokenIndex) {
          // Ensure position was created first
          handleDecreaseLiquidity(
            createDecreaseLiquidityEvent(
              tokenId,
              BigInt.fromI32(50),
