...
describe("Position Lifecycle Integration", () => {
  ...
  test("complete_position_lifecycle_with_multiple_handlers", () => {
    ...
    let tokenId = BigInt.fromI32(8001)
    ...
    handleIncreaseLiquidity(createIncreaseLiquidityEvent(
      tokenId, 
      initialLiquidity, 
      BigInt.fromI32(200), 
      BigInt.fromI32(400)
    ...
    let expectedLiquidity = initialLiquidity.plus(additionalLiquidity)
    ...
    handleDecreaseLiquidity(createDecreaseLiquidityEvent(
      tokenId,
      decreaseLiquidity,
      BigInt.fromI32(50),
      BigInt.fromI32(100)
    ...
    let finalLiquidity = expectedLiquidity.minus(decreaseLiquidity)
    ...
    validateEventCreation(tokenId, "TRANSFER", BigInt.fromI32(0), USER_1, USER_2)
    ...
    assert.fieldEquals("PositionEvent", mintEventId, "position", tokenId.toString())
    assert.fieldEquals("PositionEvent", increaseEventId, "position", tokenId.toString())
    assert.fieldEquals("PositionEvent", decreaseEventId, "position", tokenId.toString())
    assert.fieldEquals("PositionEvent", transferEventId, "position", tokenId.toString())
  })

  test("position_lifecycle_with_burn", () => {
    // Test complete lifecycle ending with NFT burn (transfer to zero address)
    
    let tokenId = BigInt.fromI32(8002)
    ...
    handleDecreaseLiquidity(createDecreaseLiquidityEvent(tokenId, liquidity, BigInt.fromI32(100), BigInt.fromI32(200)))
    
    // Burn NFT (transfer to zero address)
    handleTransfer(createTransferEvent(USER_1, Address.zero(), tokenId))

    // Validate final state
    validatePositionLifecycle(tokenId, Address.zero(), TRACKED_POOL, BigInt.fromI32(0))
    
    // All events should exist
    assert.entityCount("PositionEvent", 3) // MINT, DECREASE, TRANSFER
    
    // Verify burn transfer event
    validateEventCreation(tokenId, "TRANSFER", BigInt.fromI32(0), USER_1, Address.zero())
  })

  test("multi_position_lifecycle_independence", () => {
    // Verify that multiple positions can have independent lifecycles
    
    let tokenId1 = BigInt.fromI32(8003)
    let tokenId2 = BigInt.fromI32(8004)
    let liquidity1 = BigInt.fromI32(1500)
    let liquidity2 = BigInt.fromI32(2500)
    ...
    handleIncreaseLiquidity(createIncreaseLiquidityEvent(tokenId1, liquidity1, BigInt.fromI32(150), BigInt.fromI32(300)))
    handleIncreaseLiquidity(createIncreaseLiquidityEvent(tokenId2, liquidity2, BigInt.fromI32(250), BigInt.fromI32(500)))

    // Modify position 1 only
    handleDecreaseLiquidity(createDecreaseLiquidityEvent(tokenId1, BigInt.fromI32(500), BigInt.fromI32(50), BigInt.fromI32(100)))
    ...
    validatePositionLifecycle(tokenId1, USER_1, TRACKED_POOL, BigInt.fromI32(1000)) // 1500 - 500
  ...
  test("lifecycle_consistency_across_pool_filtering", () => {
    ...
    let trackedTokenId = BigInt.fromI32(8005)
    let liquidity = BigInt.fromI32(1000)

    setupTrackedPoolMocks(CONTRACT_ADDRESS, trackedTokenId, USER_1)

    // Complete lifecycle for tracked pool
    handleIncreaseLiquidity(createIncreaseLiquidityEvent(trackedTokenId, liquidity, BigInt.fromI32(100), BigInt.fromI32(200)))
    handleDecreaseLiquidity(createDecreaseLiquidityEvent(trackedTokenId, BigInt.fromI32(200), BigInt.fromI32(20), BigInt.fromI32(40)))
    ...
    validatePositionLifecycle(trackedTokenId, USER_2, TRACKED_POOL, BigInt.fromI32(800))
    
    // Verify all handlers maintain pool filtering consistency
    assert.fieldEquals("Position", trackedTokenId.toString(), "pool", TRACKED_POOL.toHexString())
  })
}) 